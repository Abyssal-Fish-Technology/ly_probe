...
#define PATH_LEN      256
#define BASE_ID       NTOP_BASE_ID+110
...
static V9V10TemplateElementId dumpPlugin_template[] = {
  { FLOW_TEMPLATE, NTOP_ENTERPRISE_ID, BASE_ID, PATH_LEN, ascii_format, dump_as_ascii, "DUMP_PATH", "Path where dumps will be saved" },
...
static int dumpPlugin_export(void *pluginData, V9V10TemplateElementId *theTemplateElement,
			     int direction, FlowHashBucket *bkt, char *outBuffer,
  ...
  for(i=0; dumpPlugin_template[i].templateElementId != 0; i++) {
	...
	case BASE_ID:
	  copyLen((u_char*)info->file_path, dumpPlugin_template[i].templateElementLen,
		  outBuffer, outBufferBegin, outBufferMax);
	  traceEvent(TRACE_INFO, "file_path: %s", info->file_path);
	  (*outBufferBegin) += dumpPlugin_template[i].templateElementLen;
	  break;
...
static int dumpPlugin_print(void *pluginData, V9V10TemplateElementId *theTemplateElement,
			    int direction, FlowHashBucket *bkt, 
